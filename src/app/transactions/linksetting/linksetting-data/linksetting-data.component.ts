...
@Component({
  selector: 'app-linksetting-data',
  templateUrl: './linksetting-data.component.html',
  styleUrl: './linksetting-data.component.scss'
})
export class LinksettingDataComponent implements OnInit {
...
LinkSettingList$: Observable<any[]>;
private LinkSettingListSubject: BehaviorSubject<any[]> = new BehaviorSubject<any[]>([]);
...
this.LinkSettingList$ = this.LinkSettingListSubject.asObservable();
...
RolesGET() {
  ...
  this._service
    ...
    .subscribe((response: any[]) => {
      this.LinkSettingListSubject.next(response);
...
edit(data: any) {
    this._shareds.setLinkSettingData(JSON.stringify(data));
    this.router.navigate(["/transactions/linksetting-add"]);
  }
