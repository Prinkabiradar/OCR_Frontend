...
export class AddDocumentTypeComponent implements OnInit {
  ...
  saveDocumentType() {
    ...
    this.service.saveDocumentTypeJson(payload).subscribe({
      // ✅ changed from saveDocumentType to saveDocumentTypeJson
      next: () => {
        this.saving = false;
        Swal.fire({
          icon: 'success',
      ...
      error: (err: any) => {
        ...
        const errorMessage =
          err?.error?.message || 'Failed to save Document Type';
