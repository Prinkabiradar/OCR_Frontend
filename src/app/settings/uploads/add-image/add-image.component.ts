...
export class AddImageComponent implements OnInit, OnDestroy {
  ...
  private sortSelectedFiles(): void {
    this.selectedFiles.sort((a, b) =>
      this.getComparablePath(a).localeCompare(this.getComparablePath(b), undefined, {
        numeric: true,
        sensitivity: 'base',
      }),
    );
  ...
  private getComparablePath(file: File): string {
    const relativePath = (file as any).webkitRelativePath as string | undefined;
    return (relativePath && relativePath.length ? relativePath : file.name).toLowerCase();
  }
  ...
  private proceedWithUpload() {
    ...
    this.selectedFiles.forEach((file, index) => {
      ...
      if (typeof existing !== 'number') {
        this.uploadOrderIndexByFileName.set(file.name, index + 1);
      }
    });
  ...
  loadResults(jobId: string) {
    this.service.getOcrJobResults(jobId).subscribe({
      next: (results: OcrFileResult[]) => {
        ...
        results.forEach((fileResult, index) => {
          const uploadOrder = this.getUploadOrder(fileResult.file_name, index + 1);
          const fallbackPageNumber = this.extractPageNumber(fileResult.file_name, uploadOrder);
        ...
        parsed.sort((a, b) =>
          (a.uploadOrder ?? Number.MAX_SAFE_INTEGER) - (b.uploadOrder ?? Number.MAX_SAFE_INTEGER) ||
  ...
  private extractPageNumber(fileName: string, fallbackPageNumber: number): number {
    if (!fileName) return fallbackPageNumber;

    const explicitPageMatch = fileName.match(/(?:^|[_\-\s])p(?:age)?[_\-\s]?0*(\d+)(?:\D|$)/i);
    if (explicitPageMatch) return Number(explicitPageMatch[1]);

    const trailingNumberMatch = fileName.match(/(\d+)(?=\.[^.]+$)/);
    if (trailingNumberMatch) return Number(trailingNumberMatch[1]);

    return fallbackPageNumber;
  }

  private getUploadOrder(fileName: string, defaultOrder: number): number {
    if (!fileName) return defaultOrder;

    const exact = this.uploadOrderIndexByFileName.get(fileName);
    if (typeof exact === 'number') return exact;

    const splitPdfMatch = fileName.match(/^(.*)_p\d+(\.[^.]+)$/i);
    const sourceName = splitPdfMatch
      ? `${splitPdfMatch[1]}${splitPdfMatch[2]}`
      : this.getSourceDocumentName(fileName);

    const sourceMatch = this.uploadOrderIndexByFileName.get(sourceName);
    if (typeof sourceMatch === 'number') return sourceMatch;
